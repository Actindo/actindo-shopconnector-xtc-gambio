...
function import_product( $product )
{
  ...
  if( act_shop_is(SHOP_TYPE_GAMBIOGX) )
    require_once ('gm/classes/GMProductUpload.php');
  ...
  if( $n )
  {
    ...
    if( is_null($pp['categories_id']) || (is_null($pp['verify_cat_id']) && $pp['categories_id']!=0) )
    {
      ...
      {
        return array( 'ok' => FALSE, 'errno' => EXDEV, 'error'=>'Produkt da, aber Kategoriezuordnung defekt und irreparabel.' );
      }
    }
  ...
  if( isset($product['lft']) )
    $p['manufacturers_id'] = $products['lft'];
  ...
  if( is_array($product['shop']['images']) && count($product['shop']['images']) )
  {
    foreach( $product['shop']['images'] as $num => $image )
    {
      ...
      if( act_shop_is(SHOP_TYPE_GAMBIOGX) )
      {
        ...
        // fix for #48313: always append artid+idx to image name
        $pieces = explode('.', $image['image_name']);
        $extension = array_pop($pieces);
        $pid = !empty($p['products_id']) ? $p['products_id'] : preg_replace('/[^a-zA-Z0-9_\-]/', '', $p['products_model']); // on initial import we dont have an article id yet, use ordernumber
        $image['image_name'] = sprintf('%s_%s_%d.%s', implode('.', $pieces), $pid, $idx, $extension);
      }
  ...
  if( $n )
  {
    ...
    if( !$res1 )
    {
      return array( 'ok' => FALSE, 'errno' => EIO, 'error' => 'Fehler beim setzen der Primärkategorie' );
    }
  }
  else
  {
  ...
  if( ( act_shop_is(SHOP_TYPE_XTCOMMERCE) || act_shop_is(SHOP_TYPE_GAMBIOGX) ) )
  {
    ...
    foreach( $groups as $status_id )
    {
      ...
      if( is_array($product['preisgruppen'][$status_id]) )
      {
        ...
        $i = 0;
        while(++$i && isset($product['preisgruppen'][$status_id]['preis_range'.$i])) {
  ...
  if( is_array($product['shop']['attributes']) )
  {
    ...
    if( !$res )
      return array( 'ok' => FALSE, 'errno' => EIO, 'error' => 'Fehler beim verknüpfen der Attribute mit dem Artikel' );
  }
...
function _do_import_content( &$product, $products_id )
{
  ...
  if( is_array($product['shop']['content']) )
  {
    ...
    $group_ids = '';
    if(true && act_shop_is(SHOP_TYPE_XTCOMMERCE)) { // fix for #63343 - user group visibility
        $group_ids = array('c_all_group');
        $res = act_db_query('DESCRIBE ' . TABLE_PRODUCTS);
        while($row = mysql_fetch_row($res)) {
            if(0 === strpos($row[0], 'group_permission_')) {
                $group_ids[] = sprintf('c_%d_group', substr($row[0], 17));
            }
        }
        mysql_free_result( $res );
    ...
    foreach( $product['shop']['content'] as $_idx => $content )
    {
      ...
      else if( $content['type'] == 'file' )
      {
        ...
        {
          $warning[] = "Content: Datei '{$fn}' konnte nicht vollständig geschrieben werden (written=".var_dump_string($written).", size=".var_dump_string($content['content_file_size']).")!";
          continue;
        }
...
function _do_set_article_attributes( $products_id, $product, $products_tax_class_id, $separator=null, $combination, &$options, &$values, $just_lager=FALSE )
{
  ...
  $attributeFields = actindo_get_table_fields(TABLE_PRODUCTS_ATTRIBUTES);
  $doEans = in_array('gm_ean', $attributeFields);
  ...
  foreach( $combination as $_name_id => $_values )
  {
    ...
    foreach( $_values as $_value_id => $attrib )
    {
      ...
      if( !$just_lager )
      {
        ...
        if( ( act_shop_is(SHOP_TYPE_XTCOMMERCE) || act_shop_is(SHOP_TYPE_GAMBIOGX) ) )
        {
            $sql = "INSERT INTO ".TABLE_PRODUCTS_ATTRIBUTES." SET `products_id`={$products_id}, `options_id`={$shop_options_id}, `options_values_id`={$shop_values_id}, ".
            ...
            if($doEans) {
                // attribute ean, will only work on single tier attributes (not on combined attributes)
                $attr_full_model = $product['art_nr'] . $separator . $attrib['attributes_model'];
                if(isset($product['shop']['attributes']['combination_advanced'][$attr_full_model])) {
                    $m = &$product['shop']['attributes']['combination_advanced'][$attr_full_model];
                    if(isset($m['shop']['art']['products_ean'])) {
                        $sql .= sprintf(', `gm_ean` = "%s"', esc($m['shop']['art']['products_ean']));
                    }
                }
            }
